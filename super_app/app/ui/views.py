...
class MainWindow(ctk.CTk):
    def __init__(self):
        ...
        self.btn_notes.grid(row=6, column=0, padx=PAD_SIDEBAR, pady=10, sticky="ew")
    ...
    def show_notes_view(self):
        ...
        ctk.CTkLabel(container, text="Блокнот", font=FONT_TITLE).pack(pady=10)
        ...
        entry_search = ctk.CTkEntry(top_frame, width=250, placeholder_text="Поиск...")
        entry_search.pack(side="left", padx=5)

        def on_search():
            query = entry_search.get().strip()
            notes = manager.search_notes(query) if query else manager.get_all_notes()
            self._render_notes_list(notes_frame, notes, manager)

        ctk.CTkButton(top_frame, text="🔍", width=40, height=BTN_HEIGHT,
                       command=on_search).pack(side="left", padx=5)

        combo_tag = ctk.CTkComboBox(
            top_frame, values=["Все теги"] + manager.get_all_tags(), width=140
        )
        combo_tag.set("Все теги")
        combo_tag.pack(side="left", padx=5)

        def on_filter():
            tag = combo_tag.get()
            notes = manager.filter_by_tag(tag) if tag != "Все теги" else manager.get_all_notes()
            self._render_notes_list(notes_frame, notes, manager)

        ctk.CTkButton(top_frame, text="Фильтр", width=80, height=BTN_HEIGHT,
                       command=on_filter).pack(side="left", padx=5)

        ctk.CTkButton(
            top_frame, text="➕ Новая", width=BTN_ACTION_WIDTH, height=BTN_HEIGHT,
            command=lambda: self._add_note_dialog(manager)
        ).pack(side="right", padx=5)

        stats = ctk.CTkLabel(
            container, text=f"Заметок: {manager.get_notes_count()}",
            font=FONT_SMALL, text_color=COLOR_TEXT_MUTED
        )
        stats.pack(padx=20, anchor="w")

        notes_frame = ctk.CTkScrollableFrame(container, height=350)
        notes_frame.pack(fill="both", expand=True, padx=20, pady=5)

        self._render_notes_list(notes_frame, manager.get_all_notes(), manager)

    def _render_notes_list(self, parent, notes, manager):
        for w in parent.winfo_children():
            w.destroy()
        if not notes:
            ctk.CTkLabel(parent, text="Нет заметок").pack(pady=10)
            return
        for note in notes:
            card = ctk.CTkFrame(parent, fg_color=COLOR_CARD_BG)
            card.pack(fill="x", pady=3, padx=5)

            header = ctk.CTkFrame(card, fg_color="transparent")
            header.pack(fill="x", padx=8, pady=(6, 0))

            ctk.CTkLabel(header, text=note["title"], font=FONT_SUBTITLE,
                         anchor="w").pack(side="left")
            ctk.CTkLabel(header, text=note["updated_at"],
                         font=FONT_SMALL, text_color=COLOR_TEXT_MUTED).pack(side="right")

            preview = note["content"][:120] + ("..." if len(note["content"]) > 120 else "")
            ctk.CTkLabel(card, text=preview, font=FONT_SMALL,
                         anchor="w", wraplength=600).pack(fill="x", padx=8, pady=2)

            tags_str = ", ".join(note.get("tags", []))
            if tags_str:
                ctk.CTkLabel(card, text=f"🏷 {tags_str}", font=FONT_SMALL,
                             text_color=COLOR_PRIMARY, anchor="w").pack(fill="x", padx=8)

            btn_row = ctk.CTkFrame(card, fg_color="transparent")
            btn_row.pack(fill="x", padx=8, pady=(0, 6))
            ctk.CTkButton(
                btn_row, text="✏️", width=30, height=BTN_HEIGHT,
                command=lambda nid=note["id"]: self._edit_note_dialog(manager, nid)
            ).pack(side="left", padx=2)
            ctk.CTkButton(
                btn_row, text="🗑️", width=30, height=BTN_HEIGHT,
                fg_color=COLOR_DANGER, hover_color=COLOR_DANGER_HOVER,
                command=lambda nid=note["id"]: self._delete_note_and_refresh(nid, manager)
            ).pack(side="left", padx=2)

    def _add_note_dialog(self, manager: NoteManager):
        dialog = ctk.CTkToplevel(self)
        dialog.title("Новая заметка")
        dialog.geometry("450x420")
        dialog.grab_set()

        ctk.CTkLabel(dialog, text="Заголовок:").pack(pady=(10, 0))
        row_title = ctk.CTkFrame(dialog, fg_color="transparent")
        row_title.pack(fill="x", padx=20)
        entry_title = ctk.CTkEntry(row_title, width=300)
        entry_title.pack(side="left", padx=2)
        ctk.CTkButton(row_title, text="📋", width=40, height=BTN_HEIGHT,
                       command=lambda: entry_title.insert("end", dialog.clipboard_get())
                       ).pack(side="left", padx=2)

        ctk.CTkLabel(dialog, text="Содержание:").pack()
        textbox = ctk.CTkTextbox(dialog, width=350, height=180)
        textbox.pack(pady=5)
        ctk.CTkButton(dialog, text="📋 Вставить в содержание", width=200, height=BTN_HEIGHT,
                       command=lambda: textbox.insert("end-1c", dialog.clipboard_get())
                       ).pack(pady=2)

        ctk.CTkLabel(dialog, text="Теги (через запятую):").pack()
        row_tags = ctk.CTkFrame(dialog, fg_color="transparent")
        row_tags.pack(fill="x", padx=20)
        entry_tags = ctk.CTkEntry(row_tags, width=300, placeholder_text="работа, идеи, важно")
        entry_tags.pack(side="left", padx=2)
        ctk.CTkButton(row_tags, text="📋", width=40, height=BTN_HEIGHT,
                       command=lambda: entry_tags.insert("end", dialog.clipboard_get())
                       ).pack(side="left", padx=2)
        ...
        def on_save():
            ...
            if not title or not content:
                ctk.CTkLabel(dialog, text="Заполните заголовок и содержание!",
                             text_color=COLOR_DANGER).pack()
                return
    ...
    def _edit_note_dialog(self, manager: NoteManager, note_id: int):
        note = manager.get_note_by_id(note_id)
        if not note:
            return
        dialog = ctk.CTkToplevel(self)
        dialog.title("Редактирование заметки")
        dialog.geometry("450x420")
        dialog.grab_set()
        ...
        row_title.pack(fill="x", padx=20)
        entry_title = ctk.CTkEntry(row_title, width=300)
        entry_title.insert(0, note["title"])
        entry_title.pack(side="left", padx=2)
        ctk.CTkButton(row_title, text="📋", width=40, height=BTN_HEIGHT,
                       command=lambda: entry_title.insert("end", dialog.clipboard_get())
                       ).pack(side="left", padx=2)
        ...
        row_tags = ctk.CTkFrame(dialog, fg_color="transparent")
        row_tags.pack(fill="x", padx=20)
        entry_tags = ctk.CTkEntry(row_tags, width=300)
        entry_tags.insert(0, ", ".join(note.get("tags", [])))
        entry_tags.pack(side="left", padx=2)
        ctk.CTkButton(row_tags, text="📋", width=40, height=BTN_HEIGHT,
                       command=lambda: entry_tags.insert("end", dialog.clipboard_get())
                       ).pack(side="left", padx=2)
        ...
        def on_save():
            ...
            if not title or not content:
                ctk.CTkLabel(dialog, text="Заполните заголовок и содержание!",
                             text_color=COLOR_DANGER).pack()
                return
            manager.edit_note(note_id, title, content, tags)
            dialog.destroy()
            self.show_notes_view()
    ...
    def _delete_note_and_refresh(self, note_id: int, manager: NoteManager):
